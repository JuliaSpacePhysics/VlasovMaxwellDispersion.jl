...
let g = SUITE["Maxwellian"]
    s = NormalizedSpecies(-1.0, 1.0, Maxwellian(vth=(1.2, 0.9)))
    for kp in (0.1, 1.0)
        k = Wavenumber(kp, 0.4)
...
let g = SUITE["separable"]
    ...
    ssep = prepare(NormalizedSpecies(-1.0, 1.0, SeparableVDF(
        Maxwellian(vth=(vthq, vthp));
        para=14vthp, perp=14vthq
    )))
...
let g = SUITE["kappa"]
    ...
    for κ in (6, 6.00001)   # integer (residue) vs non-integer (₂F₁) parallel path
        s = NormalizedSpecies(-1.0, 0.7, BiKappa(vth=(1.2, 0.9), kappa=κ))
        g["bikappa=$κ"] = CAV.@benchmarkable contribution($s, $ω, $k)
    end
    pbk = prepare(NormalizedSpecies(-1.0, 0.7,
        ProductBiKappa(vth=(1.2, 0.9), kappa=(4, 6))
    ))
...
let g = SUITE["Nonrelativistic/lowrank"]
    ...
    kw = (para=20vthz, perp=20vthp, rtol=1.0e-10)
...
let g = SUITE["Relativistic"], regime = Relativistic()
    ...
    for μ in (2.0, 40.0), kp in (0.7, 3.5)
        f0(w, u) = exp(-μ * γ(w, u))
        L = sqrt((1 + 25 / μ)^2 - 1)
        vdf = prepare(CoupledVDF(f0; para=L, perp=L, regime))
        ω = 0.3 + 0.05im
        k = Wavenumber(kp, 0.4)
