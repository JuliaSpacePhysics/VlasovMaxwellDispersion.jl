@testitem "generic analytic hilbert reproduces Z" begin
    ...
    for ζ in (1.5 + 0.8im, 0.3 + 0.05im, 1.2 - 0.3im) # upper, near-real, lower (Landau)
        @test abs(hilbert(g, ζ; lower=-20.0, upper=20.0) - Z(ζ)) < 1e-10
    end
end
...
@testitem "reduced SeparableVDF(Gaussian) electrostatic root matches Maxwellian" begin
    ...
    ωf = solve(LocalDispersionProblem(Plasma(Species(-1.0, Pi2, f)), k, 1.2 - 0.1im)).omega
    ωm = solve(LocalDispersionProblem(Plasma(Species(-1.0, Pi2, Maxwellian(vth))), k, 1.2 - 0.1im)).omega
    @test abs(ωf - ωm) < 1e-8
end
...
@testitem "reduced SeparableVDF bump-on-tail is unstable (Im ω > 0)" begin
    ...
    p = Plasma(Species(-1.0, 1.0, SeparableVDF(fbump; lower=-12.0, upper=14.0)))
