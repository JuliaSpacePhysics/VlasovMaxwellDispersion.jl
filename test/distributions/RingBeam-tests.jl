@testitem "Ring closures match generic SeparableVDF" begin
    ...
    for (Ω, kz, kp, vth_para, vth_perp, vd, ω, vr) in cases
        k = Wavenumber(kp, kz)
        gyro = Maxwellian(; vth_para, vth_perp, vd, vr)
        literal = GaussianRing(; vth_para, vth_perp, vd, vr)
        χ(d) = contribution(NormalizedSpecies(Ω, 1.0, d), ω, k)
        @test χ(gyro) ≈ χ(ringsep(vth_para, vth_perp, vd, vr))
        @test χ(literal) ≈ χ(rbsep(vth_para, vth_perp, vd, vr))
    end
end
...
@testitem "Parallel propagation (k⊥=0): ring energy survives β=0 fallback" begin
    ...
    ringsep(vth_para, vth_perp, vd, vr) = SeparableVDF(
        Maxwellian(; vth_para, vth_perp, vd, vr);
        para=(vd - 8vth_para, vd + 8vth_para), perp=vr + 9 * vth_perp / sqrt(2)
    )
    ...
    for (vthpar, vthperp, vd) in ((0.1, 0.12, 0.0), (0.1, 0.1, 0.05)), vr in (0.15, 0.4)
        ...
        @test χ(gyro) ≈ χ(ringsep(vthpar, vthperp, vd, vr))
        @test χ(literal) ≈ χ(rbsep(vthpar, vthperp, vd, vr))
    end
end
