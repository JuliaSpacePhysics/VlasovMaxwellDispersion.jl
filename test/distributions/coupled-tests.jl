...
@testitem "CoupledVDF(Gaussian) ≡ bi-Maxwellian (oblique)" begin
    ...
    cpl = CoupledVDF(mx; para = 10vthp, perp = 10vthq)
...
@testitem "CoupledVDF Newberger (A) ≡ HarmonicSum (B) for inseparable f₀" begin
    ...
    cpl = CoupledVDF(g0; para = 8.0, perp = 6.0)
...
@testitem "Relativistic CoupledVDF carries Bernstein term (anisotropic χ_zz)" begin
    mx = Maxwellian(vth=(0.05, 0.1))  # narrow anisotropic Gaussian
    L = 0.6
    cpl = CoupledVDF(mx; para = L, perp = L)
    cpl_rel = CoupledVDF(mx; para = L, perp = L, regime = Relativistic())
...
@testitem "CoupledVDF perp lower bound skips the empty ring core" begin
    vthpar, vthperp, vr = 0.1, 0.05, 0.6          # vr/vth⊥=12 ⇒ core density ~e⁻¹⁴⁴
    f0 = Maxwellian(; vth = (vthperp, vthpar), vr) # callable gyro-ring density
    pk = (para = 8vthpar,)
    full = CoupledVDF(f0; pk..., perp = vr + 9vthperp)
    cut = CoupledVDF(f0; pk..., perp = (vr - 6vthperp, vr + 9vthperp))
