@testitem "Maxwellian -> cold limit" begin
    ...
    epsilon_cold = dielectric(Plasma(Species(Omega_e, Pi2, ColdVDF())), omega, k)
    for vth in (1e-2, 1e-3, 1e-4)
        epsilon_hot = dielectric(Plasma(Species(Omega_e, Pi2, Maxwellian(vth))), omega, k)
...
@testitem "Langmuir wave: electrostatic_det matches standard Z dispersion" begin
    ...
    r_es = VM.muller(f_es, omega_BG - 1e-3, omega_BG + 1e-3, omega_BG + 1e-3im)
    r_std = VM.muller(f_std, omega_BG - 1e-3, omega_BG + 1e-3, omega_BG + 1e-3im)

    @test !isnan(r_es)
    @test isapprox(r_es, r_std; atol=1e-9)        # k*lambdaD ~ 0.71: damped root, exact Z-formula match
...
@testitem "Langmuir wave: Bohm-Gross + weak Landau damping" begin
    ...
    r_es = VM.muller(f_es, seed - 1e-3, seed + 1e-3, seed + 1e-3im)
    r_std = VM.muller(f_std, seed - 1e-3, seed + 1e-3, seed + 1e-3im)

    @test klD < 0.3
    @test isapprox(r_es, r_std; atol=1e-9)
    @test isapprox(real(r_es), real(omega_BG); rtol=0.02)  # Bohm-Gross, weak-damping regime
    @test imag(r_es) < 0  # damped (Landau)
    @test imag(r_es) / real(r_es) < -1e-3                  # damping resolvable, not numerical noise
end
