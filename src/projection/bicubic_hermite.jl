...
# x is the FIRST axis (v⊥), y the second (v∥). Q packs corner data
# (value / ∂y·hy / ∂x·hx / ∂x∂y·hx·hy); M·Q·Mᵀ is the normalized-coord coeff
# matrix, then de-normalized by hx^{A-1}hy^{B-1}.
...
function fit_grid(::BicubicHermite, x, y, F)
    np, nq = length(x), length(y)
    @assert size(F) == (np, nq)
    Fx = [_dcentral(x, view(F, :, j), i) for i in 1:np, j in 1:nq]
    Fy = [_dcentral(y, view(F, i, :), j) for i in 1:np, j in 1:nq]
    Fxy = [_dcentral(x, view(Fy, :, j), i) for i in 1:np, j in 1:nq]  # ∂⊥ of ∂∥
    coeffs = Matrix{SMatrix{4,4,Float64,16}}(undef, np - 1, nq - 1)
    for i in 1:(np-1), j in 1:(nq-1)
        hx, hy = x[i+1] - x[i], y[j+1] - y[j]
