...
"""
    DispersionFunction(plasma, k; closure=HarmonicSum(), deflate=true, scale=1.0)
...
at the seed).
"""
struct DispersionFunction{L, P, K, C, M, S}
    plans::L
    plasma::P
    k::K
    closure::C
...
function DispersionFunction(plasma, k; closure = HarmonicSum(), deflate = true, scale = 1.0)
    plans = map(s -> plan_contribution(s, k; closure), NormalizedPlasma(plasma).species)
    return DispersionFunction(plans, plasma, k, closure, _curlcurl(k), scale, deflate)
end
...
function (D::DispersionFunction)(ω)
    ω̃² = complex(ω)^2
    d = if D.deflate
        det(ω̃² * I + _guarded_sum(p -> scaled_contribution(p, ω), D.plans) + D.curl)
    else
        det(I + _guarded_sum(p -> p(ω), D.plans) + D.curl / ω̃²)
    end
    return d / D.scale
end
...
function exact(D::DispersionFunction)
    (; plasma, k, closure, scale, deflate) = D
    return deflate ?
        (ω -> det(wave_dispersion_tensor(plasma, ω, k; closure)) / scale) :
        (ω -> det(dispersion_tensor(plasma, ω, k; closure)) / scale)
end
...
function DispersionFunction(prob::DispersionProblem)
    ...
    scale = isfinite(s) && s > 0 ? s : one(s)
    return DispersionFunction(prob.plasma, prob.k; closure = prob.closure, deflate = false, scale)
end
