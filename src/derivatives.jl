...
@inline _dwrt1(f, u, v) = ForwardDiff.derivative(t -> f(_holo_t(t, u), v), real(u))  # ∂/∂(1st)
@inline _dwrt2(f, u, v) = ForwardDiff.derivative(t -> f(u, _holo_t(t, v)), real(v))  # ∂/∂(2nd)
@inline _dwrt(f, x) = ForwardDiff.derivative(t -> f(_holo_t(t, x)), real(x))         # 1-arg
