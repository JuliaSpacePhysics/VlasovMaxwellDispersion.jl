...
# The `ω` box spans `Re ω ∈ [0, 9] ωce` and straddles the real axis; the beam growth rates
# reach `~0.3 ωce`, so the box needs a generous imaginary extent.

kunit = wpe / wce                      # k·λₑ → k c/ωce
region = (-1.0 - 0.6im, 9.0 + 0.6im)
...
# (red/blue/green in order of decreasing peak `γ`). Each branch is drawn over the full `k`
# window where it stays inside the `ω` box; the strong Landau damping beyond the unstable
# range pushes the red and green branches out of the box before `k·λₑ = 35`.
