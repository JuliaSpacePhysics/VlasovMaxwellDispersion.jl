...
# deliberately heavy `m_e = 10⁻² m_p` — at quasi-perpendicular propagation the electron
# dynamics matter, so the reduced mass must be kept.
#
# The shell is a raw `f₀(p⊥, p∥)`, so it is written as a function of the plasma's
# [`scales`](@ref): shell speed `2 v_A`, spread the species' own `vth`, both dimensionless
# there, and both set the surrogate's truncation box. `T` on such a species is the thermal
# scale the closure reads, not a claim about the shell's second moment.
...
λp = scales(plasma).vA                        # λ_p = v_A/ω_cp in units of c/ω_cp
region = (0.05 - 0.06im, 7.8 + 0.12im)
geom = AngleSweep(k = range(0.3, 12.5, 128) ./ λp, theta = 89.5u"°")
...
klp(b) = [sqrt(abs2(k)) * λp for k in b.k]
