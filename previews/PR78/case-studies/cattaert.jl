...
# ## Plasma setup
#
# Normalized to the electron gyrofrequency `ωce` — `ref` defaults to the only species.
# `T = 2555 eV` gives `vth = √(2qT/mₑ) = 0.1 c`.

θ = deg2rad(30)
plasma = Plasma(
    Species(Electron(), ProductBiKappa(kappa_para = 1, kappa_perp = 200.0); n = 2.43e6u"m^-3", T = 2555u"eV"),
    B0 = 1.0e-6u"T",
)
...
# `vtp` is the perpendicular kappa `θ⊥ = √(1−1/κ⊥)·vth`, not the thermal speed itself,
# so it is built from `vth` rather than taken from `scales(plasma).species[1].rho`.
