...
Two shorthands recur. Anisotropic parameters (`vth`, `T`, `beta`, `kappa`) take a scalar
or a `(⊥, ∥)` pair — positional, or the order-free `(perp=…, para=…)`. Integration bounds
(`para`, `perp`) take a `(lo, hi)` tuple or a bare half-width: `para=x` ⇒ `(-x, x)`,
`perp=x` ⇒ `(0, x)`. Drifting or ring-shifted `f₀` is not centered on zero — give it the tuple.
...
pl = NormalizedSpecies(-1.0, 4.0, Maxwellian(vth=(0.04, 0.02), vd=0.0))
...
ring  = Maxwellian(vth=0.1, vr=0.3)
shell = GaussianRing(vth=0.1, vr=0.3)
...
pl = NormalizedSpecies(-1.0, 1.0, CoupledVDF(g; perp=6.0, para=6.0))

# Arbitrary separable f₀ = f⊥(p⊥)·f∥(p∥) — e.g. a product-bi-kappa
κ, vth = 6.0, 0.05
fκ(x) = (1 + x^2/(κ*vth^2))^(-κ)
pl = NormalizedSpecies(1.0, 1.0, SeparableVDF(fκ, fκ; perp=12vth, para=12vth))
