...
*pure-parallel* moments (`Z,Z'` of the bi-Maxwellian, [Maxwellian.md]). The `f⊥`/`f⊥′` split of `D_m`
...
$$
\chi=\frac{\Pi^2}{\omega^2}\sum_n\chi_n,\qquad
\chi_n=\begin{pmatrix}
...
\tfrac n\beta\Sigma_0^{(1)} & i\,\Sigma_1^{(1)} & \chi_n^{zz}
\end{pmatrix},\quad
\chi_n^{zz}=n\Omega_0\,P_0^\partial𝖬_F^2+(\omega-n\Omega_0)\,P_0𝖬_T^1
$$

(`m` rises with the entry's `p∥` power; the `33` entry is the resummed Bernstein term). This is where
`P⊥` enters χ — linearly, once per entry. A new VDF is then **only** a new `{P_j,P_j^∂}` recipe; the
