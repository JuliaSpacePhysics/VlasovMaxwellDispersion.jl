...
Two master functions close the primitive integrals, and in each the derivative form needs *no* new integral (since `f'=−2(p−v_d)f/p_th²`).
...
The perp moments $\{P_j,P_j^\partial\}$ ($j=0,1,2$) are the *only* `f⊥`-dependent input to χ: the shared
...
The clean closure survives a **perpendicular ring shift** $p_r$. Gyrophase averaging of a 2-D Maxwellian *beam* centred at $\mathbf p_r=p_r\hat{\mathbf x}$. A
gyrotropic $f_\perp$ depends only on the magnitude $p_\perp=|\mathbf p_\perp|$, but the physical beam
is a shifted 2-D Gaussian in the *vector* $\mathbf p_\perp$; averaging it over gyrophase $\phi$
(with $\mathbf p_\perp=p_\perp(\cos\phi,\sin\phi)$, so $|\mathbf p_\perp-\mathbf p_r|^2=p_\perp^2+p_r^2-2p_\perp p_r\cos\phi$):

$$
f_\perp^{ring}(p_\perp)=\Big\langle\frac{e^{-|\mathbf p_\perp-\mathbf p_r|^2/p_{\mathrm{th}\perp}^2}}{\pi p_{\mathrm{th}\perp}^2}\Big\rangle_\phi
=\frac{e^{-(p_\perp^2+p_r^2)/p_{\mathrm{th}\perp}^2}}{\pi p_{\mathrm{th}\perp}^2}\;
\frac1{2\pi}\!\int_0^{2\pi}\!e^{\,2p_r p_\perp\cos\phi/p_{\mathrm{th}\perp}^2}d\phi,
$$
...
$$
f_\perp^{ring}(p_\perp)=\frac1{\pi p_{\mathrm{th}\perp}^2}
\exp\!\Big(-\frac{p_\perp^2+p_r^2}{p_{\mathrm{th}\perp}^2}\Big)\,
I_0\!\Big(\frac{2p_r p_\perp}{p_{\mathrm{th}\perp}^2}\Big),
...
The two views matter: because $f_\perp^{ring}$ is a *bona-fide 2-D-plane* Gaussian (just gyro-averaged),
its perp moments are full-plane integrals → Weber closes them. For $p_r\gg p_{\mathrm{th}\perp}$,
$I_0(x)\sim e^x/\sqrt{2\pi x}$ ⇒ $f_\perp^{ring}\propto e^{-(p_\perp-p_r)^2/p_{\mathrm{th}\perp}^2}$, the
shifted Gaussian — but only *gyrotropized*; the literal magnitude-Gaussian (last section) is the **$\phi=0$ slice**, not the average, and loses the 2-D-plane structure (hence no Weber closure).

**Base moment closes by a second Weber integral.** Summing the perp generating function
`Σ_n J_n(z)^2 e^{-in\chi}=J_0(2z\sin\tfrac\chi2)` against `f_⊥^{ring}` and applying Weber's
*same-order* integral `∫_0^∞ t e^{-pt^2}I_0(at)J_0(ct)\,dt=\tfrac1{2p}e^{(a^2-c^2)/4p}J_0(ac/2p)`
collapses everything (the `e^{-p_r^2/\dots}` prefactor cancels):
...
- `λ→0` (cold parallel-of-thermal): `Γ_{n+m}(0)=\delta_{n+m,0}` ⇒ `Γ_n^{ring}=J_n^2(\Lambda_r)` (cold ring).
...
`K_n` is **also** closed. Its `v⊥³`-Weber is `−∂_p` of the *same-order* `I_0 J_0` integral, and the
...
+2\sigma^2\Lambda_r\,\partial_{\Lambda_r}\Gamma_n^{ring}\;},\qquad \sigma^2=\tfrac12 p_{\mathrm{th}\perp}^2,
$$
...
`f_⊥(p_⊥)=e^{−(p_⊥−p_r)²/p_{th⊥}²}/N_⊥`, ring momentum `p_r=v_{dr}`, width `p_{th⊥}` (the paper's `c_⊥`),
...
all-positive *modified*-Bessel envelope — exactly the $I_k$-tower the literal ring sums to — before
...
- **Large-argument asymptotic (closed form) — *fails* for χ.** Using
  ...
  oscillation = the Bernstein structure). The *low* harmonics close well (`P_0` to `1e-3` at `Λr=12`), but
...
- **Stable `besselj` at fixed nodes.** A fixed quadrature on the ring weight with library `besselj` (its own
  large-`z` asymptotics ⇒ no cancellation) *is* accurate, but the integrand oscillates `~(p_r/p_{th⊥})^{-1}Λr`
...
Expanding the *shift* instead, $e^{2p_r p_⊥/p_{th⊥}^2}=\sum_j (2p_r/p_{th⊥}^2)^j p_⊥^{\,j}/j!$, gives
...
Full-tensor `χ` (`RingBeam` vs `SeparableVDF`): ~1e-9 and **~400–470× faster** for `\Lambda_r\lesssim8`,
breaking (≈30%) by `\Lambda_r\approx12`.

> Implemented as `RingBeam(; vth_par, vth_perp, vd, vr)` (Route A, full tensor); `vr=0` reduces to the
> bi-Maxwellian. Past `\Lambda_r\approx10` use `SeparableVDF`. The `Maxwellian(;vr)` `I_0`-ring stays the
> choice when a `≤1.5%` gyrotropic-vs-literal model difference is acceptable.